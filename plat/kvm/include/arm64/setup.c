...
static void _dtb_get_psci_method(void)
{
	...
	rc = fdt_get_psci_method(_libkvmplat_cfg.dtb, &fdtmethod);

	if (rc >= 0 && !strcmp(fdtmethod, "hvc"))
		smcc_psci_call = smcc_psci_hvc_call;
	else if (rc  >= 0 && !strcmp(fdtmethod, "smc"))
...
void _libplat_start(void *dtb_pointer)
{
	...
	_libplat_init_console(dtb_pointer);
	...
	_libplat_newstack((uint64_t) _libkvmplat_cfg.bstack.end,
